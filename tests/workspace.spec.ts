...
test('starts empty and does not show errors or parameter overrides', async ({ page }) => {
  await expect(page.getByRole('textbox', { name: 'XML input', exact: true })).toHaveText('');
  await expect(page.getByRole('textbox', { name: 'XSLT stylesheet', exact: true })).toHaveText('');
  await expect(page.getByTestId('run-transform')).toBeDisabled();
...
test('default, empty-string override, then default again', async ({ page }) => {
  ...
  await expect(page.getByLabel('Override title', { exact: true })).not.toBeChecked();
  await page.getByLabel('Override title', { exact: true }).check();
  await expect(page.getByLabel('Value for title')).toHaveValue('');
...
test('shows stale output and keeps it visible after a malformed XML error', async ({ page }) => {
  ...
  await expect(page.getByText('Inputs changed. Run again to update this result.')).toBeVisible();
  await page.getByTestId('run-transform').click();
  await expect(page.getByRole('alert')).toContainText('Transformation needs attention'); await expect(page.getByRole('alert')).toContainText('previous successful result');
  await expect(output(page)).toContainText('original');
});
test('formats simple nested XML without changing downloads or mixed content', async ({ page }) => {
  ...
  await expect(output(page)).toContainText('<b>1</b>'); await expect(page.getByRole('alert')).toHaveCount(0);
  ...
  const nextEvent = page.waitForEvent('download'); await page.getByRole('button', { name: 'Download', exact: true }).click();
  expect(await readFile((await (await nextEvent).path())!, 'utf8')).toBe(raw);
});
...
test('namespaces are auto-detected and XPath distinguishes nodes and scalar results', async ({ page }) => {
  await page.getByRole('tab', { name: 'Inspect & XPath' }).click();
  await fill(page, 'XML input', '<orders xmlns="urn:orders"><order id="1"/><order id="2"><name>Sam</name></order></orders>');
  await page.getByLabel('XPath expression').fill('//ns:order'); await page.getByRole('button', { name: 'Evaluate XPath' }).click();
  await expect(page.getByText('2 matching nodes', { exact: true })).toBeVisible();
...
test('invalid XPath and duplicate namespace prefixes produce recoverable errors', async ({ page }) => {
  ...
  await page.getByLabel('XPath expression').fill('//*['); await page.getByRole('button', { name: 'Evaluate XPath' }).click(); await expect(page.getByRole('alert')).toContainText('Could not evaluate XPath');
  ...
  await page.getByRole('button', { name: '+ Add binding', exact: true }).click();
  await page.getByLabel('Namespace prefix 2', { exact: true }).fill('p'); await page.getByLabel('Namespace URI 2', { exact: true }).fill('urn:b');
  await page.getByRole('button', { name: 'Evaluate XPath' }).click(); await expect(page.getByRole('alert')).toContainText('duplicated');
});
...
test('examples and new workspace require confirmation before clearing user work', async ({ page }) => {
  await load(page, '<private/>', identity); page.once('dialog', d => d.dismiss()); await page.getByRole('button', { name: 'New workspace', exact: true }).click();
  await expect(page.getByRole('textbox', { name: 'XML input', exact: true })).toContainText('private');
  await page.getByRole('button', { name: 'Examples', exact: true }).click(); page.once('dialog', d => d.dismiss()); await page.getByTestId('example-orders').click(); await page.getByRole('button', { name: 'Close dialog' }).click();
  await expect(page.getByRole('textbox', { name: 'XML input', exact: true })).toContainText('private');
...
test('preview blocks scripts, remote resources and meta refresh while keeping raw output', async ({ page }) => {
  ...
  await expect(frame.locator('script')).toHaveCount(0); await expect(frame.locator('[src], [href], meta[http-equiv="refresh"]')).toHaveCount(0);
  expect(await page.evaluate(() => 'previewEscaped' in window)).toBe(false); expect(external).toEqual([]); await expect(page.locator('iframe')).toHaveAttribute('sandbox', '');
});
test('rejects DTDs and imports, and recovers with valid input', async ({ page }) => {
  await load(page, '<!DOCTYPE root [<!ENTITY x "private">]><root>&x;</root>', identity); await page.getByTestId('run-transform').click(); await expect(page.getByRole('alert')).toContainText('DTDs');
  await load(page, '<root/>', wrapXslt('<xsl:include href="https://preview-test.invalid/stylesheet"/>')); await page.getByTestId('run-transform').click(); await expect(page.getByRole('alert')).toContainText('External stylesheets');
...
test('external document() cannot access the network', async ({ page }) => {
  ...
  await page.getByTestId('run-transform').click(); await expect(page.getByRole('alert')).toBeVisible({ timeout: 15000 }); expect(external).toEqual([]);
});
test('file opening enforces size and UTF-8 limits without wiping existing input', async ({ page }) => {
  ...
  await page.getByLabel('Open XML file', { exact: true }).setInputFiles({ name: 'large.xml', mimeType: 'application/xml', buffer: Buffer.alloc(1024 * 1024 + 1, 65) });
  await expect(page.getByText('Use a UTF-8 file smaller than 1 MiB.', { exact: true })).toBeVisible(); await expect(page.getByRole('textbox', { name: 'XML input', exact: true })).toContainText('from file');
  await page.getByLabel('Open XML file', { exact: true }).setInputFiles({ name: 'invalid.xml', mimeType: 'application/xml', buffer: Buffer.from([0xc3, 0x28]) });
  await expect(page.getByText('The file is not valid UTF-8. Convert its encoding before opening it.', { exact: true })).toBeVisible(); await expect(page.getByRole('textbox', { name: 'XML input', exact: true })).toContainText('from file');
});
